  ...
  const exportOrdersToExcel = () => {
    try {
      const ordersData = orders.map(order => ({
        ...
        'Original Amount': order.original_amount ? `₹${order.original_amount.toLocaleString()}` : `₹${order.total_amount.toLocaleString()}`,
        'Promotion Code': order.promotion_code || 'None',
        'Discount Amount': order.discount_amount ? `₹${order.discount_amount.toLocaleString()}` : '₹0',
        'Final Amount': `₹${order.total_amount.toLocaleString()}`,
        'Savings': order.discount_amount ? `₹${order.discount_amount.toLocaleString()}` : '₹0',
